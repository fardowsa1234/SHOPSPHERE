...
function AddProduct() {
  ...
  const getCategories = () => {
    setCategories([]);
    axios.get("http://localhost:1337/api/Category")
      .then(response => setCategories(response.data.data.map(item => ({
        id: item.id,
        ...item.attributes
      }))));
  };
  ...
  const addProduct = async (event) => {
    ...
    try {
      ...
      await axios.post("http://localhost:1337/api/Product", formData, {
